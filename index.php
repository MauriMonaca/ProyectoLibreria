			...
			<div class="row no-gutters">
				<section class="col-12 px-5 pt-lg-3 books-wrap py-3" id="books-catalogue">
					...
					<div class="row">
						...
						$totalBooks = mysqli_num_rows($query);
						$books_per_page = 9;
						$pages = $totalBooks / $books_per_page;
						$pages = ceil($pages);
						$pag = "";
						if (isset($_GET['page'])){
							$pag = $_GET['page'];
						...
						$init = ($pag-1) * $books_per_page;
						$initString = strval($init);
						$books_per_page_string = strval($books_per_page);
						$sql_pages = " SELECT * FROM libros ORDER BY editorial LIMIT $initString, $books_per_page_string ";
						$query_pages = mysqli_query($link,$sql_pages);
						while ($cell = mysqli_fetch_array($query_pages)) {
						?>
					  			...
					  			<div class="card-body p-0">
								   <div class="d-flex justify-content-between mx-lg-3">
								   	<div class="d-flex flex-column justify-content-between mt-2 p-0">
								   	  <h3 class="card-title mb-0 mx-n2"><?php echo $cell['nombre_libro'] . "&nbsp (" . $cell['anio'] . ")";  ?></h3>
					...
					<div aria-label="Page navigation" class="books-pagination mt-3">
					  <ul class="pagination">
					    <li class="page-item <?php echo $_GET['page'] <= 1 ? 'd-none' : '' ?>"><a class="page-link m-0" href="index.php?page=<?php echo $_GET['page']-1; ?>">Anterior</a></li>
					    <?php for($i = 0;$i < $pages; $i++) { ?>
 					    <li class="page-item <?php echo $_GET['page'] == $i+1 ? 'active' : '' ?>"><a class="page-link m-0" href="index.php?page=<?php echo $i+1; ?>"><?php echo $i+1; ?></a></li>
 						<?php } ?>
					    <li class="page-item <?php echo $_GET['page'] >=  $pages ? 'd-none' : ''?>"><a class="page-link m-0" href="index.php?page=<?php echo $_GET['page']+1; ?>">Siguiente</a></li>
					  </ul>
					</div>
				</section>
			</div>
